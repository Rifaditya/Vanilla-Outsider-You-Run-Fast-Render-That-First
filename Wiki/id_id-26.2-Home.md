...
## 📖 Ringkasan Subproyek: Jangkar MC 26.2

Selamat datang di dokumentasi teknis **Vanilla Outsider: You Run Fast, Render That First** untuk **Minecraft 26.2**.

Dikompilasi untuk Minecraft `26.2`, Fabric Loader `>=0.18.4`, dan Java 25+. Mengoptimalkan `SectionTaskDynamicQueue` dan sinkronisasi aturan via DasikLibrary `>=1.8.0`.
...
| :--- | :--- | :--- |
| **Client Meshing** | [[Anisotropic Prioritization|id_id-26.2-Anisotropic-Prioritization]] | Directional dot-product distance bias, EMA smoothing, zero-allocation render polling |
| **Server Worldgen** | [[Chunk Generation Biasing|id_id-26.2-Chunk-Generation-Biasing]] | Predictive forward tickets (`TicketType.PLAYER_LOADING`), MSPT watchdog, lateral trimming |
| **Configuration** | [[Configuration & GameRules|id_id-26.2-Configuration-and-GameRules]] | Dynamic namespaced GameRules, `/yourunfast` command suite (`help`, `status`, `set`) |
| **Architecture** | [[Architecture & Mixins|id_id-26.2-Architecture-and-Mixins]] | Mixin inspection (`SectionTaskDynamicQueueMixin`), FastUtil data structures, zero-GC fast path |
