...
## 📖 Descripción del subproyecto: Ancla MC 26.2

Bienvenido a la documentación técnica de **Vanilla Outsider: You Run Fast, Render That First** para **Minecraft 26.2**.

Compilado para Minecraft `26.2`, Fabric Loader `>=0.18.4` y Java 25+. Optimiza la cola `SectionTaskDynamicQueue` y sincroniza GameRules con DasikLibrary `>=1.8.0`.
...
| :--- | :--- | :--- |
| **Client Meshing** | [[Anisotropic Prioritization|es_es-26.2-Anisotropic-Prioritization]] | Directional dot-product distance bias, EMA smoothing, zero-allocation render polling |
| **Server Worldgen** | [[Chunk Generation Biasing|es_es-26.2-Chunk-Generation-Biasing]] | Predictive forward tickets (`TicketType.PLAYER_LOADING`), MSPT watchdog, lateral trimming |
| **Configuration** | [[Configuration & GameRules|es_es-26.2-Configuration-and-GameRules]] | Dynamic namespaced GameRules, `/yourunfast` command suite (`help`, `status`, `set`) |
| **Architecture** | [[Architecture & Mixins|es_es-26.2-Architecture-and-Mixins]] | Mixin inspection (`SectionTaskDynamicQueueMixin`), FastUtil data structures, zero-GC fast path |
