...
## 📖 Vue d'ensemble du sous-projet : Ancre MC 26.3

Bienvenue sur la documentation technique de **Vanilla Outsider: You Run Fast, Render That First** pour **Minecraft 26.3**.

Compilé pour Minecraft `26.3`, Fabric Loader `>=0.18.4` et Java 25+. Cible `SectionTaskDynamicQueue` et DasikLibrary `>=1.8.0`.
...
| :--- | :--- | :--- |
| **Client Meshing** | [[Anisotropic Prioritization|fr_fr-26.3-Anisotropic-Prioritization]] | Directional dot-product distance bias, EMA smoothing, zero-allocation render polling |
| **Server Worldgen** | [[Chunk Generation Biasing|fr_fr-26.3-Chunk-Generation-Biasing]] | Predictive forward tickets (`TicketType.PLAYER_LOADING`), MSPT watchdog, lateral trimming |
| **Configuration** | [[Configuration & GameRules|fr_fr-26.3-Configuration-and-GameRules]] | Dynamic namespaced GameRules, `/yourunfast` command suite (`help`, `status`, `set`) |
| **Architecture** | [[Architecture & Mixins|fr_fr-26.3-Architecture-and-Mixins]] | Mixin inspection (`SectionTaskDynamicQueueMixin`), FastUtil data structures, zero-GC fast path |
