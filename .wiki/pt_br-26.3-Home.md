...
## 📖 Visão Geral do Subprojeto: Âncora MC 26.3

Bem-vindo à documentação técnica de **Vanilla Outsider: You Run Fast, Render That First** no **Minecraft 26.3**.

Compilado para Minecraft `26.3`, Fabric Loader `>=0.18.4` e Java 25+. Otimiza a fila `SectionTaskDynamicQueue` e utiliza DasikLibrary `>=1.8.0`.
...
| :--- | :--- | :--- |
| **Client Meshing** | [[Anisotropic Prioritization|pt_br-26.3-Anisotropic-Prioritization]] | Directional dot-product distance bias, EMA smoothing, zero-allocation render polling |
| **Server Worldgen** | [[Chunk Generation Biasing|pt_br-26.3-Chunk-Generation-Biasing]] | Predictive forward tickets (`TicketType.PLAYER_LOADING`), MSPT watchdog, lateral trimming |
| **Configuration** | [[Configuration & GameRules|pt_br-26.3-Configuration-and-GameRules]] | Dynamic namespaced GameRules, `/yourunfast` command suite (`help`, `status`, `set`) |
| **Architecture** | [[Architecture & Mixins|pt_br-26.3-Architecture-and-Mixins]] | Mixin inspection (`SectionTaskDynamicQueueMixin`), FastUtil data structures, zero-GC fast path |
