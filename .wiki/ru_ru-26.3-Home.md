...
## 📖 Обзор подпроекта: якорная версия MC 26.3

Добро пожаловать в техническую документацию **Vanilla Outsider: You Run Fast, Render That First** для **Minecraft 26.3**.

Данная версия собрана под Minecraft `26.3`, Fabric Loader `>=0.18.4` и Java 25+. Мод оптимизирует клиентскую очередь `SectionTaskDynamicQueue` и использует DasikLibrary `>=1.8.0` для синхронизации правил.
...
| :--- | :--- | :--- |
| **Client Meshing** | [[Anisotropic Prioritization|ru_ru-26.3-Anisotropic-Prioritization]] | Directional dot-product distance bias, EMA smoothing, zero-allocation render polling |
| **Server Worldgen** | [[Chunk Generation Biasing|ru_ru-26.3-Chunk-Generation-Biasing]] | Predictive forward tickets (`TicketType.PLAYER_LOADING`), MSPT watchdog, lateral trimming |
| **Configuration** | [[Configuration & GameRules|ru_ru-26.3-Configuration-and-GameRules]] | Dynamic namespaced GameRules, `/yourunfast` command suite (`help`, `status`, `set`) |
| **Architecture** | [[Architecture & Mixins|ru_ru-26.3-Architecture-and-Mixins]] | Mixin inspection (`SectionTaskDynamicQueueMixin`), FastUtil data structures, zero-GC fast path |
